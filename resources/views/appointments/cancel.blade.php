...
@section('content')
          ...
          <div class="card shadow">
            ...
            <div class="card-body">
              ...
              @endif
           @if($role=='paciente')   
            <p> Se cancelará tu cita con el médico <b>{{$appointment->doctor->name}}</b> 
          ...
          @else($role=='admin')
          <p> Se cancelará la cita medica del paciente  <b>{{$appointment->patient->name}}</b>
          que seria atendido por el doctor <b>{{$appointment->doctor->name}}</b>,  
            (especialidad <b>{{$appointment->specialty->name}}</b>)
             para el dia <b>{{$appointment->scheduled_date}}</b>
            a las <b>{{$appointment->scheduled_time_12}}</b></p>
          @endif
